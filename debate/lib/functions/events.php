...
if (isset($state)) {
	...
	if (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_SESSION['level'] >= 2) {
		if (isset($full)) {
			...
			try {
				$db = new DbConn;
				$stmt = $db->conn->prepare("SELECT * FROM events WHERE id = :event");
